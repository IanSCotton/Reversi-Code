...
•	The player can decide on the size of the board they will be playing on, from f"{2*2-12*12}"\
...
•	fill_chosen_player_box(master_frame, background_colour): this fills the box to show the chosen player for each side, it sets the column and row sizes, and uses new function place_stats(row, column, columnspan, font_size, text, setting) which places a label with a specific font, font_size and background at a specific place in the grid taking up a certain number of columns and adds drop down menus with create_custom_drop_down_menu if setting=”label”, if it’s entry then it creates an entry that can be changed by clicking on it and runs update_global_ai_tree_layers(*args) which allows a new valid number to be entered and updates AI_tree_layers for that side. Returns result\
